<%@ page language="java" contentType="text/html; charset=EUC-KR"
    pageEncoding="EUC-KR"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=EUC-KR">
</head>
<body>
<div id="wrap">
	<div class="content">
		<p>▶ 상품 소개 페이지</p>
		<div>
			<div class="floatL">> 허위 매물 또는 문제의 소지가 있는 내용의 판매 글은 관리자에게 신고 하여주세요.</div>
			<div class="floatR">[신고하기]</div>
		</div>
		<br>
		<br>
		<div>
			<table bgcolor=#e6fddf border=1>
				<colgroup>
					<col width="300px">
					<col width="430px">
				</colgroup>
				<tbody>
					<tr>
						<td colspan=2>
							상 품 명 : ASUS 지포스 GTX680 ENGT680 D5 2GB 판매자 : ASUS 판매자등급 : 골드 <img class="" alt="골드등급" src="/images/user_grade_gold.gif">
						</td>
					</tr>
					<tr>
						<td rowspan=4>
							<table>
								<colgroup>
									<col width="300px">
								</colgroup>
								<tbody>
									<tr>
										<td>
											<img class="" alt="큰 이미지" src="/images/product001_pic.gif">
										</td>
									</tr>
									<tr>
										<td>
											<img class="" alt="작은 이미지" src="/images/product002_pic.gif">
											<img class="" alt="작은 이미지" src="/images/product002_pic.gif">
											<img class="" alt="작은 이미지" src="/images/product002_pic.gif">
											<img class="" alt="크게보기" src="/images/btn/extend_pic.gif">
										</td>
									</tr>
									<tr>
										<td>
											<table>
												<colgroup>
													<col width="60px">
													<col width="240px">
												</colgroup>
												<tbody>
													<tr>
														<td>
															<img class="" alt="경고" src="/images/caution.gif">
														</td>
														<td>
															세이프컴스 내 모든 상거래 책임은 거래당사자에게 있으며, 거래전 신중하게 결정하시기 바랍니다.
														</td>
													</tr>
												</tbody>
											</table>
										</td>
									</tr>
								</tbody>
							</table>
						</td>
						<td>
							카테고리 : 컴퓨터 > PC주요부품 > VGA > 지포스<br>
							상품번호 : 201294109284<br>
							상 품 명 : ASUS 지포스 GTX680 ENGT680 D5 2GB<br>
							판매수량 : 1개<br>
							판매가격 : 650,000원
						</td>
					</tr>
					<tr>
						<td>
							상품상태 : ○ 중고 ○ 신품<br>
							무상 AS : ○ 불가 ○ 가능 (구입일자 00년 00월  00일)<br>
							배송방법 : 택배발송<br>
							배송요금 : ○ 판매자부담 ○ 구매자부탐 (예상요금 : 5,000원)
						</td>
					</tr>
					<tr>
						<td>
							판매자ID : ASUS<br>
							이름 : 아서스<br>
							가입일 : 2012년 4월 25일<br>
							판매자등급 : 골드등급 <img class="" alt="골드등급" src="/images/user_grade_gold.gif">
						</td>
					</tr>
					<tr>
						<td>
							<table>
								<colgroup>
									<col width="150px">
									<col width="120px">
									<col width="170px">
								</colgroup>
								<tbody>
									<tr>
										<td>신용카드로 구매</td>
										<td>\ 676,000원</td>
										<td rowspan=5><img class="" alt="세이프컴스구매" src="/images/btn/btn_safecoms_order.gif"></td>
									</tr>
									<tr>
										<td>실시간 계좌이체로 구매</td>
										<td>\ 663,000원</td>
									</tr>
									<tr>
										<td>무통장 입금으로 구매</td>
										<td>\ 650,000원</td>
									</tr>
									<tr>
										<td>적립금으로 구매</td>
										<td>\ 650,000원</td>
									</tr>
									<tr>
										<td colspan=2>*적립금이 있을 경우 삭감 후 총 금액을 표기</td>
									</tr>
								</tbody>
							</table>
						</td>
					</tr>
				</tbody>
			</table>
		</div>
		<br>
		<div>
			세이프컴스는 해당 제품에 대한 정확성에 대한 보장을 하지 않습니다.<br>
			구체적인 기능과 사양은 구매전 꼼꼼히 확인하시기 바랍니다.<br>
			※판매자 연락처는 구매자에게만 공개됩니다.
		</div>
		<br>
		<div>
			상품내용 ...
		</div>
		<br>
		<div>
			ID : sdlink(국승욱) 등급 <img class="" alt="골드등급" src="/images/user_grade_gold.gif"> 2012-08-22 15:30:20  답글 수정 삭제
			<br>
			댓글 내용
		</div>
		<br>
		<div>
			ID : sdlink(국승욱) 등급 <img class="" alt="골드등급" src="/images/user_grade_gold.gif"> 2012-08-22 15:30:20  답글 수정 삭제
			<br>
			댓글 내용
		</div>
		<br>
		<div>
			<table>
				<colgroup>
					<col width="700px">
					<col width="100px">
				</colgroup>
				<tbody>
					<tr>
						<td colspan=2>
							▷ 댓글
						</td>
					</tr>
					<tr>
						<td rowspan=2>
							<textarea rows=4 style="width:98%;"></textarea>
						</td>
						<td>
							<img class="" alt="골드등급" src="/images/btn/btn_board_input.gif">
						</td>
					</tr>
					<tr>
						<td>
							<img class="" alt="골드등급" src="/images/btn/btn_board_cancle.gif">
						</td>
					</tr>
				</tbody>
			</table>
		</div>
	</div>
</div>
</body>
</html>